...
function Slide () {
  ...
  const [isUlClassNameOn, setUlClassNameOn] = useState('slides')
  ...
  function updateWidth() {
    ...
    console.log(newWidth); // 4110px

    ulRef.current.style.width = newWidth;
    setInitialPosition()
    setTimeout(()=> {setUlClassNameOn('slides animated')}, 100)
    setUlClassNameOn('slides animated')
  }
  ...
  // console.log(currentSlideIndex, slideCount); // 4 4
    {
      setTimeout(function () {
      // ulRef.classList.remove('animated')
      setUlClassNameOn('slides')
    ...
    setTimeout(function () {
    // ulRef.classList.add('animated')
    setUlClassNameOn('slides animated')
  }, 600);
  }

  const cloneSlide = playList.map(el => {
    return (
      <li className='slide clone'
    ...
    <div>
      ...
      <section className='slide-container'>
        <ul className={isUlClassNameOn} ref={ulRef} >
          {cloneSlide}
          {playList.map((el, i) => <li className={`slides img${i}`}
            style={{ backgroundImage: `url(${el.img})`}}/>)}
          {cloneSlide}
        </ul>
      </section>
...
//   function setInitialPosition () {
//     // 이동할 translate 의 value : - (기본 li 의 width + li의 margin-left ) * 기존 li 개수 px
//     const initialTranslateValue = `-${(slideWidth + slideMargin)}` * playList.length;
